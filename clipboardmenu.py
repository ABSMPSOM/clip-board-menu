...
# =========================================================
# PATHS  — everything lives next to the .exe
# =========================================================

def _base_dir():
    """Returns the folder containing the running .exe (or .py)."""
...
def _exe_path():
    ...
    # When running as .py, register python + script so it still works
...
class PermissionDialog(tk.Toplevel):
    ...
    Asks for background-monitoring and auto-startup consent.
    result = {"background": bool, "startup": bool} or None if dismissed.
    """
    ...
    def _build(self):
        ...
        tk.Label(hdr, text="📋", bg=BG_ROOT, fg=FG_TEXT,
                 font=("Segoe UI Emoji", 26)).pack()
        tk.Label(hdr, text="Clipboard History Manager",
                 bg=BG_ROOT, fg=FG_TEXT,
        ...
        tk.Label(body,
                 text="Allow the following permissions to get started:",
                 bg=BG_WIDGET, fg=FG_TEXT,
                 font=("Consolas", 10, "bold"),
                 anchor="w").pack(fill="x", pady=(0, 16))

        # ── checkbox 1 — background monitoring ───────────
        ...
        deny = tk.Button(
            btn_row, text="Deny All",
            font=("Consolas", 10), bg=BG_ROOT, fg=FG_DIM,
            activebackground=BG_ROOT, activeforeground=FG_TEXT,
        ...
        allow = tk.Button(
            btn_row, text="Allow & Continue  →",
            font=("Consolas", 11, "bold"), bg=BG_ACCENT, fg=FG_TEXT,
            activebackground=BG_ROOT, activeforeground=FG_TEXT,
    ...
    def _make_checkbox(self, parent, var, icon, title, subtitle):
        ...
        # custom checkbox drawn on canvas
        cb_canvas = tk.Canvas(row, width=22, height=22,
                              bg=BG_WIDGET, highlightthickness=0, bd=0)
        cb_canvas.pack(side=tk.LEFT, padx=(0, 12), anchor="n", pady=2)

        def _draw(checked):
            cb_canvas.delete("all")
            fill = BG_ACCENT if checked else BG_ROOT
            cb_canvas.create_rounded_rectangle = lambda *a, **k: None  # no-op guard
            cb_canvas.create_rectangle(0, 0, 22, 22, fill=fill,
                                       outline=BG_ACCENT, width=2)
            if checked:
        ...
        tk.Label(title_row, text=icon, bg=BG_WIDGET, fg=FG_TEXT,
                 font=("Segoe UI Emoji", 12)).pack(side=tk.LEFT, padx=(0, 6))
        tk.Label(title_row, text=title, bg=BG_WIDGET, fg=FG_TEXT,
                 font=("Consolas", 10, "bold")).pack(side=tk.LEFT)

        tk.Label(text_col, text=subtitle, bg=BG_WIDGET, fg=FG_DIM,
...
def run_first_time_setup(app):
    """
    Shows PermissionDialog on first run.
    Saves choices to config.json.
    Returns the config dict.
    """
    cfg = load_config()

    if cfg.get("setup_done"):
        # already configured — just make sure registry matches saved pref
    ...
    # Save so we never show it again
    cfg.update({
        "setup_done":  True,
...
class TrayManager:
    ...
    def show_icon(self):
        ...
        self._icon = pystray.Icon(APP_NAME, img,
                                  "Clipboard History Manager", menu)
...
class DarkScrollbar(tk.Canvas):
    ...
    def _drag(self, event):
        ...
        h = self.winfo_height()
        p = max(0.0, min(1.0 - (self._last - self._first),
                         self._drag_pos + (event.y - self._drag_y) / h))
...
class CustomTitleBar(tk.Frame):
    def __init__(self, parent, win, tray, title="App", **kw):
        ...
        self._maximized = False; self._pre_max = None
        self._dx = self._dy = 0

        icon_lbl  = tk.Label(self, text="📋", bg=BG_WIDGET, fg=FG_TEXT,
                             font=("Segoe UI Emoji", 14))
        icon_lbl.pack(side=tk.LEFT, padx=(14, 4))

        title_lbl = tk.Label(self, text=title, bg=BG_WIDGET, fg=FG_TEXT,
                             font=("Consolas", 13, "bold"))
        title_lbl.pack(side=tk.LEFT)

        sub_lbl   = tk.Label(self, text="auto-saves · 30-day expiry · 1 000 items",
                             bg=BG_WIDGET, fg=FG_DIM, font=("Consolas", 8))
        sub_lbl.pack(side=tk.LEFT, padx=14)

        btn_kw = dict(bg=BG_WIDGET, fg=FG_TEXT, activeforeground=FG_TEXT,
                      bd=0, relief="flat", font=("Consolas", 12),
                      width=3, cursor="hand2")

        self._cb = tk.Button(self, text="✕", activebackground=CLR_CLOSE,
                             command=self._close, **btn_kw)
        ...
        self._mb = tk.Button(self, text="⬜", activebackground=BG_ACCENT,
                             command=self._toggle_max, **btn_kw)
        ...
        self._nb = tk.Button(self, text="─", activebackground=BG_ACCENT,
                             command=self._minimise, **btn_kw)
...
def make_button(parent, text, command):
    btn = tk.Button(parent, text=text, command=command,
                    font=("Consolas", 11, "bold"), width=16, height=2,
...
def create_ui(app, tray):
    CustomTitleBar(app, app, tray,
                   title="Clipboard History Manager").pack(fill="x")
    ...
    tk.Label(left, text="HISTORY", bg=BG_ROOT, fg=FG_DIM,
             font=("Consolas", 9, "bold"), anchor="w").pack(fill="x", padx=4, pady=(0,4))

    lb_border = tk.Frame(left, bg=BG_ACCENT, padx=1, pady=1)
    lb_border.pack(fill="both", expand=True)

    listbox = tk.Listbox(lb_border, width=56, height=30,
                         bg=BG_WIDGET, fg=FG_TEXT, font=("Consolas", 10),
                         selectbackground=BG_SELECT, selectforeground=FG_SELECT,
    ...
    listbox.bind("<MouseWheel>",
                 lambda e: listbox.yview_scroll(-1 if e.delta > 0 else 1, "units"))

    # RIGHT
    right = tk.Frame(main_frame, bg=BG_ROOT)
    right.pack(side=tk.RIGHT, fill="both", expand=True, padx=(16,0))
    tk.Label(right, text="PREVIEW", bg=BG_ROOT, fg=FG_DIM,
             font=("Consolas", 9, "bold"), anchor="w").pack(fill="x", padx=4, pady=(0,4))

    pb_border = tk.Frame(right, bg=BG_ACCENT, padx=1, pady=1)
    pb_border.pack(fill="both", expand=True)

    preview_box = tk.Text(pb_border, bg=BG_WIDGET, fg=FG_TEXT,
                          insertbackground=FG_TEXT, font=("Consolas", 11),
    ...
    make_button(btn_frame, "📋  Copy Again",
                lambda: copy_selected_clipboard(listbox)).pack(side=tk.LEFT, padx=(0,8))
    make_button(btn_frame, "🗑  Delete",
                lambda: delete_selected_clipboard(listbox, preview_box)).pack(side=tk.LEFT, padx=(0,8))
    make_button(btn_frame, "🔄  Refresh",
                lambda: refresh_listbox(listbox, preview_box)).pack(side=tk.LEFT)

    tk.Frame(app, bg=BG_ACCENT, height=2).pack(fill="x")
    status = tk.Frame(app, bg=BG_WIDGET, pady=6)
    status.pack(fill="x")
    tk.Label(status, text="●  Monitoring clipboard",
             bg=BG_WIDGET, fg=BG_ACCENT, font=("Consolas", 9)).pack(side=tk.LEFT, padx=12)

    listbox.bind("<<ListboxSelect>>",
                 lambda e: show_selected_clipboard(e, listbox, preview_box))
...
def main():
    ...
    app.withdraw()   # hide until setup is done
    ...
    # If user denied background permission, close to tray is skipped
    # (minimise still works, but we don't force tray-only mode)
